...
- `No image_generation_call result returned` with HTTP 200
  Action: use streaming extraction (`--stream on`) or keep default `--stream auto` so the script can read `response.image_generation_call.partial_image` / completed events.
