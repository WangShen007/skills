...
2. Choose a size that matches the target:
   ...
   - Landscape: `1536x1024`
   - If prompt contains `高清/超清/4K/8K`, the helper auto-sends `quality=high` (unless explicitly overridden).
3. Run `scripts/generate_gateway_image.py` (it now uses streaming-first auto mode by default).
...
- `--quality auto|low|medium|high`: defaults to `auto` (`auto` promotes to `high` when prompt includes `高清/超清/4K/8K`)
- `--stream auto|on|off`: defaults to `auto` (`auto` tries SSE stream first, then falls back to non-stream JSON)
...
- Uses streaming-first extraction in `auto` mode to avoid gateways that omit `output.image_generation_call.result` in non-stream responses
...
If non-stream requests complete but `output` is empty (for example `No image_generation_call result returned`), use `--stream on` (or keep default `--stream auto`).
