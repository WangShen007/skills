...
def main():
    ...
    if status not in {"dispatched"} | RECORDABLE_PAGE_STATUSES:
        raise SystemExit(
            f"{page['page_id']} cannot be reset from status {status}; "
            "reset only applies to dispatched, recorded, or candidate pages"
        )
    ...
    page["dispatch"] = None
    page["result"] = None
    page["candidate_id"] = None
    page["candidate_sha256"] = None
