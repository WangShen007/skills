...
def cmd_next(args: argparse.Namespace) -> int:
    ...
    unfinished = [
        ...
        for page in pages
        if page.get("status") not in RECORDABLE_PAGE_STATUSES | {"accepted"}
    ]
    if unfinished:
        payload = {
            "run_dir": str(run_dir),
    ...
    if pages and all(page.get("status") == "accepted" for page in pages):
        payload = {
            ...
            "run_status": "complete",
            "next_command": None,
            "agent_focus": "Run is complete and immutable; start a new run for material corrections.",
        }
        return print_json(payload) if args.json else _print_next_text(payload)

    current_candidate = deck.get("current_candidate")
    if current_candidate and any(page.get("status") in CANDIDATE_PAGE_STATUSES for page in pages):
        payload = {
            "run_dir": str(run_dir),
            "stage": "candidate_review",
            "run_status": state.get("status"),
            "candidate_id": current_candidate.get("candidate_id"),
            "candidate_sha256": current_candidate.get("pptx_sha256"),
            "next_command": f"{cli_prog()} run accept {run_dir} --review-summary <review_summary.json>",
            "agent_focus": "Inspect the current Office-rendered candidate and reviewer summary; accept only when all review evidence names its hash.",
        }
        return print_json(payload) if args.json else _print_next_text(payload)

    if current_candidate and state.get("status") not in {"complete", "accepted"}:
        payload = {
            "run_dir": str(run_dir),
            "stage": "candidate_rebuild",
            "run_status": state.get("status"),
            "candidate_id": current_candidate.get("candidate_id"),
            "next_command": f"{cli_prog()} run finalize {run_dir} --defer-accept",
            "agent_focus": "The page was re-recorded after a candidate review; build a new immutable candidate before reviewing again.",
        }
        return print_json(payload) if args.json else _print_next_text(payload)
    ...
    payload = {
        ...
        "agent_focus": "All pages are recorded. Build and validate the final PPTX (use --defer-accept for the reviewable workflow).",
    }
    return print_json(payload) if args.json else _print_next_text(payload)
...
def cmd_page_render_office(args: argparse.Namespace) -> int:
    argv = [args.page_dir]
    if args.pptx:
        argv.extend(["--pptx", args.pptx])
    if args.manifest:
    ...
    if args.timeout is not None:
        argv.extend(["--timeout", str(args.timeout)])
    return run_script("office_render.py", argv)


def cmd_page_text_audit(args: argparse.Namespace) -> int:
    argv = [args.page_dir, "--render", args.render]
    if args.manifest:
        argv.extend(["--manifest", args.manifest])
    if args.out:
        argv.extend(["--out", args.out])
    if args.ink_threshold is not None:
        argv.extend(["--ink-threshold", str(args.ink_threshold)])
    return run_script("text_audit.py", argv)


def cmd_finalize(args: argparse.Namespace) -> int:
    argv = [args.run]
    if args.defer_accept:
        argv.append("--defer-accept")
    return run_script("finalize_deck_run.py", argv)


def cmd_accept(args: argparse.Namespace) -> int:
    return run_script("accept_deck_run.py", [args.run, "--review-summary", args.review_summary])
...
def build_parser() -> argparse.ArgumentParser:
  ...
  - page provides deterministic hints, Office rendering, text audit, and validation helpers.
...
Examples:
  ...
  editppt run accept <run> --review-summary <review_summary.json>
...
""",
    ...
    doctor = sub.add_parser(
        "doctor",
        help="Check CLI dependencies, config status, and Office renderer.",
...
CLI fallback readiness when --check-api is passed, and local Office renderer
availability for final visual QA. It cannot detect the Agent-only
image_gen.imagegen tool and does not perform a network API probe. An unavailable
Office renderer is reported separately and does not fail unrelated CLI health.
""",
        formatter_class=HELP_FORMATTER,
        epilog="""Examples:
  editppt doctor
...
""",
    ...
    finalize = run_sub.add_parser(
        "finalize",
        help="Build and validate a candidate or the final deck.",
        description="Build from recorded page manifests. Use --defer-accept to write an immutable candidate for Office-render QA; without it, preserve the legacy immediate-accept behavior.",
        formatter_class=HELP_FORMATTER,
    )
    finalize.add_argument("run", metavar="RUN", help="Run directory or deck_manifest.json path.")
    finalize.add_argument("--defer-accept", action="store_true", help="Build final/candidates/candidate-N without accepting the run.")
    finalize.set_defaults(func=cmd_finalize)

    accept = run_sub.add_parser(
        "accept",
        help="Accept the current candidate after reviewer-owned QA evidence.",
        description="Verify a pass review summary is bound to the current candidate hash, then freeze the run and copy the candidate to the configured final output path.",
        formatter_class=HELP_FORMATTER,
    )
    accept.add_argument("run", metavar="RUN", help="Run directory or deck_manifest.json path.")
    accept.add_argument("--review-summary", required=True, metavar="FILE", help="JSON review summary naming the current candidate pptx_sha256.")
...
""",
    ...
    page_render_office = page_sub.add_parser(
        "render-office",
        help="Render page.pptx through LibreOffice/soffice for Office-first calibration.",
        formatter_class=HELP_FORMATTER,
    )
    page_render_office.add_argument("page_dir", metavar="PAGE_DIR", help="Page directory containing page.pptx and manifest.json.")
    page_render_office.add_argument("--pptx", default="page.pptx", metavar="FILE", help="PPTX file relative to page_dir.")
    page_render_office.add_argument("--manifest", default="manifest.json", metavar="FILE", help="Manifest file relative to page_dir.")
    page_render_office.add_argument("--out", metavar="DIR", help="Output directory; defaults to calibration/office-render.")
    page_render_office.add_argument("--dpi", type=int, default=144, metavar="N", help="PDF rasterization DPI (default: 144).")
    page_render_office.add_argument("--timeout", type=int, default=120, metavar="SEC", help="Office conversion timeout in seconds.")
    page_render_office.set_defaults(func=cmd_page_render_office)

    page_text_audit = page_sub.add_parser(
        "text-audit",
        help="Audit configured text boxes against an Office-rendered PNG.",
        formatter_class=HELP_FORMATTER,
    )
    page_text_audit.add_argument("page_dir", metavar="PAGE_DIR", help="Page directory containing manifest.json.")
    page_text_audit.add_argument("--render", required=True, metavar="PNG", help="Office-rendered page PNG, usually calibration/office-render/slide-1.png.")
    page_text_audit.add_argument("--manifest", default="manifest.json", metavar="FILE", help="Manifest file relative to page_dir.")
    page_text_audit.add_argument("--out", metavar="FILE", help="Report path; defaults to calibration/text-audit.json.")
    page_text_audit.add_argument("--ink-threshold", type=int, default=16, metavar="N", help="Minimum RGB contrast from local background.")
    page_text_audit.set_defaults(func=cmd_page_text_audit)
