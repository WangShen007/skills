...
def office_renderer_status():
    """Report the first executable available for final Office-render QA."""
    ...
    for command in ("soffice", "libreoffice"):
        ...
        if executable:
            return {
                ...
                "command": command,
                "reason": "local Office renderer is available",
                "next": "render the finalized PPTX before claiming visual QA passed",
            }
    return {
        "ready": False,
        "executable": None,
        "command": None,
        "reason": "neither soffice nor libreoffice was found on PATH",
        "next": (
            "install LibreOffice/soffice, add its bin directory to PATH, or provide another "
            "final Office/PDF renderer; "
            "do not claim Office-render visual QA passed while unavailable"
        ),
    }
...
def collect_status(check_api=False):
    ...
    return {
        ...
        "office_renderer": office_renderer_status(),
        "text_hints": {
...
def doctor(args):
    ...
    if office["ready"]:
        print(f"Office renderer=ready ({office['executable']})")
        print(f"Office-render visual QA: available; {office['next']}")
    else:
        print("Office renderer=unavailable (soffice/libreoffice not found)")
        print(f"Office-render visual QA: unavailable; {office['next']}")
...
def main():
    ...
    doc = sub.add_parser(
        "doctor",
        help="Check Python dependencies, fallback config, and Office renderer",
    )
    doc.add_argument(
        "--check-api",
        action="store_true",
