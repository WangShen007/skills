...
RECORDABLE_PAGE_STATUSES = {"recorded", "candidate_built", "visual_review_passed"}
CANDIDATE_PAGE_STATUSES = {"candidate_built", "visual_review_passed"}
...
def save_jobs(run_dir, jobs):
    ...
    synchronize_deck_from_jobs(run_dir, jobs)


def synchronize_deck_from_jobs(run_dir, jobs):
    """Keep deck page projections aligned with the page-job source of truth."""

    path = deck_manifest_path(run_dir)
    if not path.exists():
        return
    deck = read_json(path)
    jobs_by_id = {page.get("page_id"): page for page in jobs.get("pages", [])}
    for deck_page in deck.get("pages", []):
        job = jobs_by_id.get(deck_page.get("page_id"))
        if not job:
            continue
        for key in (
            "status",
            ...
            "candidate_sha256",
            "acceptance",
            # Legacy manifests also carry agent_status. Keep that projection
            # synchronized so old runs cannot retain a stale pending value.
            "agent_status",
        ):
            if key in job:
                deck_page[key] = deepcopy(job.get(key))
        if "agent_status" in deck_page and "status" in job:
            deck_page["agent_status"] = deepcopy(job.get("status"))
    if "run_status" in jobs:
        deck["run_status"] = jobs["run_status"]
...
def update_jobs_run_status(jobs):
    ...
    if pages and all(
        page.get("status")
        in {"dispatched", "recorded", "candidate_built", "visual_review_passed", "accepted"}
        for page in pages
    ):
        jobs["run_status"] = "pages_dispatched"
    if pages and all(page.get("status") in RECORDABLE_PAGE_STATUSES | {"accepted"} for page in pages):
        if any(page.get("status") in CANDIDATE_PAGE_STATUSES for page in pages):
            jobs["run_status"] = "candidate_built"
        else:
