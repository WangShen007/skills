...
6. After the page manifest is built and `page.pptx` validates, run `editppt page render-office {{PAGE_DIR}}` when a local Office renderer is available, then run `editppt page text-audit {{PAGE_DIR}} --render {{PAGE_DIR}}/calibration/office-render/slide-1.png`. Treat audit errors as page fixes; treat audit warnings as explicit inspection prompts. Office calibration is diagnostic and does not replace the parent visual QA or candidate acceptance gate.
...
7. Run Office-first page calibration when the renderer is available: `editppt page render-office {{PAGE_DIR}}`, followed by `editppt page text-audit {{PAGE_DIR}} --render {{PAGE_DIR}}/calibration/office-render/slide-1.png`. Fix clipping, fixed-line wrapping, configured ink-size failures, and size-group mismatches before returning. Keep the report as diagnostic evidence; it cannot declare visual QA passed.
8. Perform the page-local visual pass: compare `preview.png`, the Office render, and `split_assets_contact.png` to the source by region. Check major bounds, internal primitives, connector routing, text scale/wrapping, and layer order. Deterministic validation and text-audit passing are not visual acceptance; fix visible page-local mismatches before returning.
...
Before returning, run the Final Self-Check in page-decision-tree.md once: compare preview.png, the Office render when available, and split_assets_contact.png to the source, confirm `editppt page validate {{PAGE_DIR}}` passes, confirm validation.json contains top-level `passed: true`, and confirm all required outputs exist. Page-local issues are fixed inside the current page by you before returning. Page-local evidence does not set the parent visual-QA conclusion; the parent agent owns the independent assembled-candidate visual gate, candidate-hash-bound review summary, and final `run accept` decision.
