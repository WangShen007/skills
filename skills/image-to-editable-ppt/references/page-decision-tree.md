...
- 2. Foreground asset separation
- 3. PPT native element reconstruction
- 3.4 Flat technical diagrams: structural completeness
- Final self-check
- Fix versus warning
...
- Keep deterministic runtime fitting (`fit_text`) enabled as the overflow guard for the first draft. After a box and font size have been manually calibrated against a rendered preview, set `fit_text: false` (or use explicit fit bounds) for that calibrated box; leaving global fitting enabled can shrink a dense diagram's body text systematically.
...
- Calibrate in two passes: first major geometry and connector routing, then typography and wrapping. Re-render after each pass; never accept a page because the object count is correct while its text is visibly too small or its branches are misplaced.
...
Architecture, pipeline, and research-framework diagrams often look simple because most objects are flat geometry. That simplicity makes omissions obvious. Before writing the manifest, inventory the sub-objects inside each region, not only the large cards:

- document/file bodies, folds, tabs, and overlapping window strips;
- branch rails, arrow shafts, arrowheads, dashed feedback paths, loopbacks, and junctions;
- decision diamonds, exit branches, short dividers, borders, and small status marks;
- repeated encoder/exit blocks, list rows, badges, and input/output symbols.

Draw these as native structural shapes when they are ordinary geometry. Preserve their source-pixel bounds, z-order, stroke style, and routing. A missing window strip, branch, arrowhead, or dashed path is a completeness defect even when all large panels and text boxes are present. Use the region-by-region visual gate in `references/visual-qa.md` to catch these omissions.
...
### 3.6 Text Strokes and Decoration Splitting
...
The page reconstructor checks the page once against this list and records the evidence in structured manifest fields and `validation.json`. Deterministic validation is necessary but not sufficient. The parent agent must independently run the assembled-deck visual gate in `references/visual-qa.md` after finalize; page-local self-check and parent-level visual QA are separate obligations.
...
- Corners follow 3.5; large container corners, table borders, and card borders align with the source. Corner misclassification is a current-page fix, not a low-risk warning.
- No text stroke is redrawn as a decorative shape (3.6).
- Dashboards, tables, cards, and charts are decomposed per 1.4, never screenshotted wholesale.
- Flat technical diagrams include all visible internal structural sub-objects listed in 3.4.
- Badge and circular-number groups follow the shared-box centering rule in 3.7.
- z-index follows 3.7; no text or key object is covered.
...
- A visible structural primitive is missing, a major panel is materially displaced, or a connector/branch routes to the wrong object.
