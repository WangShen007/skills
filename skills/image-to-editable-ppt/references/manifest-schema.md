...
Owner: created by `editppt prepare`; `editppt run backend` may update the image backend; `editppt run finalize` adds candidate evidence and `editppt run accept` writes formal acceptance evidence.
...
Candidate lifecycle fields are optional when reading legacy runs, but new runs
write them. `current_candidate` is the exact candidate under review;
`accepted_candidate` is populated only after `run accept` (or by the legacy
immediate-finalize path); `candidates` retains every immutable iteration. A
candidate record contains `candidate_id`, `pptx`, `pptx_sha256`, `validation`,
`validation_sha256`, `manifest_sha256`, `page_manifests`, and `status`. The
manifest digest is computed from the page manifest paths and hashes used for
that candidate.

Before `run accept`, the runtime recomputes the current page-manifest digest and
rejects a candidate whose recorded page hashes no longer match. Re-record the
changed page and build a new deferred candidate instead of accepting stale
manifest evidence.
...
`dispatch` is written by `editppt run dispatch`. It includes `execution_mode`: `"worker"` for normal page-worker dispatch and `"local"` for the parent agent's single-page local claim; older dispatch records without this field are treated as `"worker"`. A page with status `dispatched` is an active execution lease until explicit completion, failure, cancellation, or lost-worker verification; elapsed time alone does not make it lost. `result` is written by `editppt run record`. Deferred finalization sets page status `candidate_built` and binds `candidate_id`/`candidate_sha256`; acceptance briefly records `visual_review_passed` and then writes `accepted` plus `acceptance` evidence. `accepted` is written by `editppt run accept` or the compatibility `editppt run finalize` path.

`deck_manifest.json` and `page_jobs.json` are synchronized by every runtime
state write. `page_jobs.json` remains the page-state source of truth; the deck
page entries are its projection and must not retain stale `pending`/`null`
status while jobs are recorded or accepted.
...
- Keep default fitting enabled for first drafts. After the page author has manually calibrated the box and font size against a rendered preview, set `fit_text: false` for that calibrated box when stable source matching matters; otherwise the runtime may shrink dense body text during the final build.
...
Office calibration fields are optional and page-measured; they never change the
object-source contract and are not global page-type rules:

```json
{
  "id": "stage_header_1",
  "box_px": [120, 780, 900, 115],
  "font_size": 24,
  "font_size_source": "measured",
  "size_group": "stage_headers",
  "expected_lines": 1,
  "wrap_policy": "fixed",
  "target_ink_height_px": 82,
  "target_ink_width_px": 760,
  "ink_height_tolerance": 0.12,
  "ink_width_tolerance": 0.18,
  "target_font_size": 24,
  "font_size_tolerance": 0.01
}
```

`editppt page text-audit` uses these fields only when present. `size_group`
requires consistent requested font sizes; `expected_lines` with
`wrap_policy: fixed` detects unexpected Office wrapping; target ink dimensions
are compared in source-pixel units; and configured edge/size failures are
reported as deterministic errors. Missing or low-contrast ink is a warning for
human inspection, not automatic visual acceptance. Use `audit_margin_px` only
when the surrounding composition requires a larger inspection margin.
